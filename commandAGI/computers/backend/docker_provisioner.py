...
class DockerProvisioner(BaseComputerProvisioner):
    ...
    This provisioner creates and manages Docker containers for running the commandAGI daemon.
    ...
    Args:
        ...
        name_prefix: Prefix to use when generating container names (default: "commandagi-daemon")
    ...
    def _setup_local(self):
        ...
        # Add the image name
        run_cmd.append(f"commandagi-daemon:{self.version}")
    ...
    def _setup_aws_ecs(self):
        ...
        response = self.ecs_client.run_task(
            cluster="commandagi-cluster",
            taskDefinition="commandagi-daemon",
            launchType="FARGATE",
        ...
        if self._task_arn:
            ...
            try:
                self.ecs_client.stop_task(
                    cluster="commandagi-cluster", task=self._task_arn
                )
                ...
                while time.time() - start_time < self.timeout:
                    try:
                        response = self.ecs_client.describe_tasks(
                            cluster="commandagi-cluster", tasks=[self._task_arn]
                        )
