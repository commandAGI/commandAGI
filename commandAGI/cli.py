...
def grid_overlay(
    ...
    if not processors_available:
        typer.echo(
            "Error: Processors module not available. Install with: pip install commandagi[processors]"
        )
        return
...
def ocr(
    ...
    if not processors_available:
        typer.echo(
            "Error: Processors module not available. Install with: pip install commandagi[processors,pytesseract]"
        )
        return
...
def daemon(
    ...
    try:
        from commandAGI.daemon.cli import start

        typer.echo(f"Starting daemon on port {port} with backend {backend}...")
        start(port=port, backend=backend)
    except ImportError:
        typer.echo(
            "Error: Daemon module not available. Install with: pip install commandagi[daemon]"
        )
    except Exception as e:
        typer.echo(f"Error: {e}")
