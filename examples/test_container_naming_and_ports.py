...
from commandLAB._utils.network import find_free_port, _is_port_available
...
def test_find_free_port():
    ...
    assert _is_port_available(preferred_port), f"Port {preferred_port} should be available"
    ...
    with temp_fastapi_server(preferred_port):
        assert not _is_port_available(preferred_port), f"Port {preferred_port} should be occupied"
        
        # Should get a different port
        new_port = find_free_port(preferred_port=preferred_port)
        assert new_port != preferred_port, f"Expected a different port than {preferred_port}, got {new_port}"
        print(f"✓ Successfully got alternative port {new_port} when preferred port {preferred_port} was occupied")
        ...
        for p in range(min_port, max_port + 1):
            if _is_port_available(p):
                if available_port is None:
                    ...
                    stop_event = threading.Event()
                    server_thread = threading.Thread(target=run_server, args=(p, stop_event))
                    server_thread.daemon = True
                    server_thread.start()
...
def test_container_naming():
    ...
    try:
        subprocess.run(["docker", "ps", "-a", "-q", "--filter", f"name={name_prefix}"], 
                      check=True, capture_output=True, text=True)
        
        # If any containers exist with this prefix, remove them
        result = subprocess.run(["docker", "ps", "-a", "-q", "--filter", f"name={name_prefix}"], 
                              check=True, capture_output=True, text=True)
    ...
    try:
        subprocess.run(["docker", "run", "-d", "--name", name_prefix, "alpine", "sleep", "30"], 
        ...
        subprocess.run(["docker", "run", "-d", "--name", expected, "alpine", "sleep", "30"], 
                      check=True, capture_output=True)
        ...
        subprocess.run(["docker", "run", "-d", "--name", f"{name_prefix}-3", "alpine", "sleep", "30"], 
                      check=True, capture_output=True)
...
def test_provisioner_port_selection():
    ...
    with temp_fastapi_server(base_port):
        ...
        assert provisioner.daemon_port != base_port, f"Expected a different port than {base_port}"
        ...
        try:
            ...
            assert provisioner.daemon_port == available_port, f"Expected port {available_port}, got {provisioner.daemon_port}"
            print(f"✓ Provisioner selected available port {provisioner.daemon_port} from range {port_range}")
        finally:
            # Clean up the servers
            for p, stop_event, server_thread in occupied_servers:
                stop_event.set()
...
def test_multiple_provisioners():
    ...
    provisioner1 = DockerProvisioner(
        container_name="test-daemon-1",
        daemon_port=port1
    )
    
    provisioner2 = DockerProvisioner(
        container_name="test-daemon-2", 
        daemon_port=port2
    )
    ...
    assert provisioner1.daemon_port == port1, f"Expected port {port1}, got {provisioner1.daemon_port}"
    assert provisioner2.daemon_port == port2, f"Expected port {port2}, got {provisioner2.daemon_port}"
    assert provisioner3.daemon_port == port3, f"Expected port {port3}, got {provisioner3.daemon_port}"
    ...
    with temp_fastapi_server(port1):
        ...
        new_provisioner = DockerProvisioner(
            container_name="test-daemon-4",
            daemon_port=port1
        )
        
        # Mock the setup method
        new_provisioner.setup = lambda: setattr(new_provisioner, 'daemon_port', find_free_port(preferred_port=port1))
        
        # Simulate setup
        new_provisioner.setup()
        
        # Check that the provisioner found a different port
        assert new_provisioner.daemon_port != port1, f"Expected a different port than {port1}"
        print(f"✓ New provisioner selected alternative port {new_provisioner.daemon_port} when {port1} was occupied")
