...
def main():
    ...
    try:
        ...
        if displays and displays.displays:
            ...
            logger.info(f"Drag from ({start_x}, {start_y}) to ({end_x}, {end_y}) success: {success}")
        ...
        logger.info("Observation keys: %s", list(observation.keys()) if observation else "No observation data")
        ...
        logger.info(f"Keys currently pressed: {[k for k, v in keyboard_state.keys.items() if v]}")
