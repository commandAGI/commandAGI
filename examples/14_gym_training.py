#!/usr/bin/env python3
"""
commandAGI Gym Training Example

This example demonstrates how to use the commandAGI gym framework to train an agent
...
class SimpleTaskEnv(ComputerEnv):
    ...
    def get_reward(self, action: ComputerAction) -> float:
        ...
        if action.command is not None and not self.editor_opened:
            if (
                "notepad" in action.command.command.lower()
                or "gedit" in action.command.command.lower()
            ):
                reward += 2.0
...
class SimpleMockTrainableAgent(NaiveComputerAgent):
        ...
        if self.steps_taken == 0:
            ...
            return ComputerAction(
                mouse_click=MouseClickAction(
                    x=300, y=300, button=MouseButton.LEFT, move_duration=0.5
                )
            )
        elif self.steps_taken == 2:
            # Third action: Type some text
            return ComputerAction(type=TypeAction(text="Hello from training agent!"))
        else:
    ...
    def train(self, episodes: List[Episode]) -> None:
        ...
        completion_rate = sum(
            ...
            if any(step.info.get("task_completed", False) for step in episode)
        ) / len(episodes)
...
def main():
    print("commandAGI Gym Training Example")
    print("===============================")
    ...
    try:
        ...
        try:
            ...
            print(
                f"Average episode length: {sum(ep.num_steps for ep in episodes) / len(episodes):.2f} steps"
            )
            print(
                f"Average reward: {sum(agent.episode_rewards) / len(agent.episode_rewards):.2f}"
            )
            print(
                f"Completion rate: {sum(agent.completion_rate) / len(agent.completion_rate):.2%}"
            )
