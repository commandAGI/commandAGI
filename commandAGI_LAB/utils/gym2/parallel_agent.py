...
class ParallelAgent(
    BaseAgent[Dict[str, ObsType], Dict[str, ActType]], Generic[ObsType, ActType]
):
    """Manages multiple agents in parallel, with each agent identified by a string key."""

    def __init__(
        self,
        agent_factory: Callable[[], BaseAgent[ObsType, ActType]],
        agent_ids: List[str],
    ):
        ...
        self.agents: Dict[str, BaseAgent[ObsType, ActType]] = {
            agent_id: agent_factory() for agent_id in agent_ids
        }
    ...
    def act(self, observations: Dict[str, ObsType]) -> Dict[str, ActType]:
        ...
        if set(observations.keys()) != set(self.agents.keys()):
            raise ValueError(
                f"Observation keys {observations.keys()} don't match agent keys {self.agents.keys()}"
            )
    ...
    def update(self, rewards: Dict[str, float]) -> None:
        ...
        if set(rewards.keys()) != set(self.agents.keys()):
            raise ValueError(
                f"Reward keys {rewards.keys()} don't match agent keys {self.agents.keys()}"
            )
