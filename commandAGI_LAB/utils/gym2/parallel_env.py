...
from .spaces import DictSpace, Space
...
class ParallelEnv(
    Env[Dict[str, ObsType], Dict[str, ActType]], Generic[ObsType, ActType]
):
    ...
    def observation_space(self) -> Space[Dict[str, ObsType]]:
        ...
        return DictSpace(
            spaces={env_id: self._base_obs_space for env_id in self.active_envs}
        )
    ...
    def action_space(self) -> Space[Dict[str, ActType]]:
        ...
        return DictSpace(
            spaces={env_id: self._base_act_space for env_id in self.active_envs}
        )

    def __init__(
        self,
        env_factory: Callable[[], Env[ObsType, ActType]],
        num_envs: int,
    ...
    def step(
        self, action: Dict[str, ActType]
    ) -> Tuple[Dict[str, ObsType], float, bool, Dict[str, Any]]:
    ...
    def get_observation(self) -> Dict[str, ObsType]:
        ...
        return {
            env_id: self.envs[env_id].get_observation() for env_id in self.active_envs
        }
    ...
    def execute_action(self, action: Dict[str, ActType]) -> bool:
        ...
        if set(action.keys()) != set(self.active_envs):
            raise ValueError(
                f"Action keys {action.keys()} don't match active envs {self.active_envs}"
            )
        return all(
            self.envs[env_id].execute_action(action[env_id])
    ...
    def get_reward(self, action: Dict[str, ActType]) -> float:
        ...
        return sum(
            self.envs[env_id].get_reward(action[env_id]) for env_id in self.active_envs
        )
    ...
    def get_done(self, action: Dict[str, ActType]) -> bool:
        ...
        return all(
            self.envs[env_id].get_done(action[env_id]) for env_id in self.active_envs
        )

    def get_info(self) -> Dict[str, Any]:
        """Get info from active environments only."""
        return {env_id: self.envs[env_id].get_info() for env_id in self.active_envs}
