...
from commandLAB.types import (
    ComputerAction, ComputerObservation,
    MouseButton, KeyboardKey,
    ComputerActionType,
    CommandAction, KeyboardKeyPressAction, KeyboardKeysPressAction,
    KeyboardKeyDownAction, KeyboardKeysDownAction,
    KeyboardKeyReleaseAction, KeyboardKeysReleaseAction,
    KeyboardHotkeyAction,
    TypeAction,
    MouseMoveAction, MouseScrollAction,
    MouseButtonDownAction, MouseButtonUpAction,
...
class OpenRLComputerEnv(gym.Env):
    ...
    def __init__(self, env: BaseComputerEnv):
        super().__init__()
        self.env = env
        
        # Define observation space
        self.observation_space = spaces.Dict({
            'screenshot': spaces.Box(low=0, high=255, shape=(None, None, 3), dtype=np.uint8),
            'mouse_state': spaces.Dict({
                'buttons': spaces.Dict({
                    btn.value: spaces.Discrete(2) for btn in MouseButton
                }),
                'position': spaces.Box(low=0, high=float('inf'), shape=(2,), dtype=np.int32)
            }),
        ...
        self.action_space = spaces.Dict({
            ...
            'params': spaces.Dict({
                ...
                'timeout': spaces.Box(low=0, high=float('inf'), shape=(), dtype=np.float32),
                
                # Keyboard actions
                'keys': spaces.Sequence(spaces.Text(min_length=1, max_length=20)),
                'key': spaces.Text(min_length=1, max_length=20),
                'duration': spaces.Box(low=0, high=float('inf'), shape=(), dtype=np.float32),
                'text': spaces.Text(min_length=0, max_length=1000),
                
                # Mouse actions
                'x': spaces.Box(low=0, high=float('inf'), shape=(), dtype=np.int32),
                'y': spaces.Box(low=0, high=float('inf'), shape=(), dtype=np.int32),
                'move_duration': spaces.Box(low=0, high=float('inf'), shape=(), dtype=np.float32),
                'amount': spaces.Box(low=-float('inf'), high=float('inf'), shape=(), dtype=np.float32),
                'button': spaces.Discrete(len(MouseButton)),
                'press_duration': spaces.Box(low=0, high=float('inf'), shape=(), dtype=np.float32),
                'start_x': spaces.Box(low=0, high=float('inf'), shape=(), dtype=np.int32),
                'start_y': spaces.Box(low=0, high=float('inf'), shape=(), dtype=np.int32),
                'end_x': spaces.Box(low=0, high=float('inf'), shape=(), dtype=np.int32),
                'end_y': spaces.Box(low=0, high=float('inf'), shape=(), dtype=np.int32),
            })
        })
    ...
    def _process_observation(self, obs: ComputerObservation) -> Dict[str, Any]:
        ...
        if obs.get('keyboard_state'):
            processed_obs['keyboard_state'] = {
                'keys': obs['keyboard_state'].keys
            }
    ...
    def _create_action(self, action: Dict[str, Any]) -> ComputerAction:
        ...
        action_type = ComputerActionType(list(ComputerActionType)[action['action_type']])
        ...
        if action_type == ComputerActionType.COMMAND:
            ...
            computer_action['type'] = TypeAction(
                text=params['text']
            )
            ...
            computer_action['mouse_scroll'] = MouseScrollAction(
            ...
            computer_action['double_click'] = DoubleClickAction(
                ...
                press_duration=params.get('press_duration', 0.1),
                button=params.get('button', MouseButton.LEFT),
                double_click_interval_seconds=params.get('double_click_interval_seconds', 0.1)
            )
