...
def ensure_packer_template(template_path: str, template_content: Dict[str, Any]) -> None:
...
def build_kubernetes_image(
    ...
    cmd = [
        "docker", "build",
        "-t", f"commandlab-daemon-k8s:{version}",
        "--build-arg", f"VERSION={get_package_version()}",
        "-f", os.path.join(dockerfile_path, "Dockerfile"),
        "."
    ]
...
def build_all_images(
    ...
    try:
        ...
        logger.info("To build cloud images, please install Packer: https://www.packer.io/downloads")
