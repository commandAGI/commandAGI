...
class BaseComputerProvisioner(ABC):
    ...
    def setup(self) -> None:
        ...
        while provision_attempt < self.max_provisioning_retries:
            print(f"Attempt {provision_attempt + 1}/{self.max_provisioning_retries} to setup resource")
            try:
        ...
        print(f"Waiting for resource and daemon to be running (timeout: {self.health_check_timeout}s)")
        ...
        while health_check_attempt < self.max_health_retries:
            try:
                ...
                if not daemon_responsive:
                    raise RuntimeError("Resource is running but daemon is not responsive")

                # Both checks passed
                print("Resource is running and daemon is responsive")
                self._set_status(ProvisionerStatus.RUNNING)
                print(f"Resource and daemon are now running after {int(time.time() - health_check_start_time)}s")
                return
                ...
                if elapsed_seconds > self.health_check_timeout:
                    ...
                    print(f"Timeout waiting for resource and daemon to start after {elapsed_seconds}s")
                    raise TimeoutError(f"Timeout waiting for resource and daemon to start after {elapsed_seconds}s")
