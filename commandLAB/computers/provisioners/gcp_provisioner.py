...
class GCPProvisioner(BaseComputerProvisioner):
    def __init__(
        ...
        self.source_image = source_image or "projects/your-project/global/images/commandlab-daemon"
    ...
    def setup(self) -> None:
        ...
        while retry_count < self.max_retries:
            try:
                logger.info(f"Creating GCP VM {self.instance_name} in project {self.project}, zone {self.zone}")
                ...
                raise TimeoutError(f"Timeout waiting for VM {self.instance_name} to start")
                
            except Exception as e:
                retry_count += 1
                if retry_count >= self.max_retries:
                    self._status = "error"
                    logger.error(f"Failed to create GCP VM after {self.max_retries} attempts: {e}")
                    raise
                logger.warning(f"Error creating GCP VM, retrying ({retry_count}/{self.max_retries}): {e}")
                time.sleep(2 ** retry_count)  # Exponential backoff

    def teardown(self) -> None:
        self._status = "stopping"
        logger.info(f"Deleting VM {self.instance_name} from project {self.project}, zone {self.zone}")
