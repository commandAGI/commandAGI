    ...
    def setup(self) -> None:
        ...
        while retry_count < self.max_retries:
            try:
                ...
                self._status = "error"
                logger.error(f"Timeout waiting for instance {self.instance_id} to start")
                raise TimeoutError(f"Timeout waiting for instance {self.instance_id} to start")
                
            except Exception as e:
                retry_count += 1
                if retry_count >= self.max_retries:
                    self._status = "error"
                    logger.error(f"Failed to launch EC2 instance after {self.max_retries} attempts: {e}")
                    raise
                logger.warning(f"Error launching EC2 instance, retrying ({retry_count}/{self.max_retries}): {e}")
        ...
        try:
            ...
            while time.time() - start_time < self.timeout:
                try:
                    ...
                    if state == 'terminated':
                        logger.info(f"Instance {self.instance_id} successfully terminated")
                        break
