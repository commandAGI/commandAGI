...
class VirtualBoxProvisioner(BaseComputerProvisioner):
        ...
        while retry_count < self.max_retries:
            try:
                ...
                if self.vm_image:
                    ...
                    self._vboxmanage_command(["import", self.vm_image, "--vsys", "0", "--vmname", self.vm_name])
                
                # Start the VM
                logger.info(f"Starting VM {self.vm_name}")
                self._vboxmanage_command(["startvm", self.vm_name, "--type", "headless"])
                ...
                while time.time() - start_time < self.timeout:
                    if self.is_running():
                        ...
                        self._vboxmanage_command([
                            ...
                            "--natpf1", f"daemon,tcp,,{self.port},,{self.port}"
                        ])
                        
                        # Run daemon setup script
                        self._vboxmanage_command([
                            "guestcontrol", self.vm_name,
                            "run", "--exe", "/bin/bash",
                            "--username", "vagrant", "--password", "vagrant",
                            "--", "bash", "-c",
                            f"pip install commandlab[local,daemon] && python -m commandlab.daemon.daemon --port {self.port} --backend pynput"
                        ])
    ...
    def is_running(self) -> bool:
        ...
        try:
            # Get VM info
            result = self._vboxmanage_command(["showvminfo", self.vm_name, "--machinereadable"], capture_output=True)
            return "VMState=\"running\"" in result
        except Exception as e:
            logger.error(f"Error checking VM status: {e}")
            return False
    
    def _vboxmanage_command(self, args: list, capture_output: bool = False) -> Optional[str]:
