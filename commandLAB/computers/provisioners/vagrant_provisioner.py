...
class VagrantProvisioner(BaseComputerProvisioner):
        ...
        while retry_count < self.max_retries:
            try:
                ...
                if retry_count >= self.max_retries:
                    self._status = "error"
                    logger.error(f"Failed to start Vagrant VM after {self.max_retries} attempts: {e}")
                    raise
                logger.warning(f"Error starting Vagrant VM, retrying ({retry_count}/{self.max_retries}): {e}")
    ...
    def _vagrant_command(self, args: list, capture_output: bool = False) -> Optional[str]:
...
Vagrant.configure("2") do |config|
  ...
  config.vm.provision "shell", inline: <<-SHELL
    ...
    python3 -m commandlab.daemon.daemon --port {self.port} --backend pynput
  SHELL
end
"""
