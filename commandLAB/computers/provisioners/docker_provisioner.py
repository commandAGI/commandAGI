...
class DockerProvisioner(BaseComputerProvisioner):
    def __init__(
        ...
        if platform == DockerPlatform.AWS_ECS:
            ...
            if not self.subscription_id:
                raise ValueError("Subscription ID must be specified for Azure Container Instances")
        ...
        while retry_count < self.max_retries:
            try:
                ...
                if retry_count >= self.max_retries:
                    self._status = "error"
                    logger.error(f"Failed to setup container after {self.max_retries} attempts: {e}")
                    raise
                logger.warning(f"Error setting up container, retrying ({retry_count}/{self.max_retries}): {e}")
    ...
    def _setup_azure_container_instances(self):
        ...
        logger.info(f"Starting Azure Container Instance {self.container_name} in resource group {self.resource_group}")
    ...
    def _setup_gcp_cloud_run(self):
        ...
        logger.info(f"Starting GCP Cloud Run service {self.container_name} in project {self.project_id}")
    ...
    def _teardown_azure_container_instances(self):
        ...
        try:
            ...
            if poller.done():
                logger.info(f"Azure Container Instance {self.container_name} deleted successfully")
            else:
    ...
    def _teardown_gcp_cloud_run(self):
        ...
        try:
            ...
            if operation.done():
                logger.info(f"Cloud Run service {self.container_name} deleted successfully")
            else:
    ...
    def _is_azure_container_instances_running(self) -> bool:
        ...
        try:
            ...
            is_running = container_group.containers[0].instance_view.current_state.state == 'Running'
            logger.debug(f"Azure Container Instance {self.container_name} running status: {is_running}")
            return is_running
        except Exception as e:
    ...
    def _is_gcp_cloud_run_running(self) -> bool:
        ...
        try:
            ...
            logger.debug(f"Cloud Run service {self.container_name} running status: {is_running}")
            return is_running
        except Exception as e:
