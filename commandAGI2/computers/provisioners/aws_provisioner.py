...
class AWSProvisioner(BaseComputerProvisioner):
    def __init__(
        ...
        self.image_id = image_id or "ami-commandagi2"  # Default AMI ID
        self.security_groups = security_groups or ["commandagi2-daemon"]
        self.ec2 = boto3.client("ec2", region_name=self.region)

    def _provision_resource(self) -> None:
        """Provision an EC2 instance with the commandAGI2 AMI"""
        ...
        response = self.ec2.run_instances(
            ...
            UserData=f"""#!/bin/bash
                ...
                python -m commandagi2.daemon.daemon --port {self.daemon_port} --token {self.daemon_token} --backend pynput
            """,
        )
