...
from commandAGI2._utils.network import find_free_port
...
class DockerProvisioner(BaseComputerProvisioner):
    ...
    This provisioner creates and manages Docker containers for running the commandAGI2 daemon.
    ...
    Args:
        ...
        name_prefix: Prefix to use when generating container names (default: "commandagi2-daemon")
    ...
    def __init__(
        ...
        port_range: Optional[
            Tuple[int, int]
        ] = None,  # Only used for LOCAL platform to handle port collisions
        daemon_token: Optional[str] = None,
        container_name: Optional[str] = None,
        name_prefix: str = "commandagi2-daemon",
    ...
    def _find_next_available_container_name(self) -> str:
        ...
        'commandagi2-daemon' and 'commandagi2-daemon-1' exist, it will return 'commandagi2-daemon-2'.
    ...
    def _setup_local(self):
        ...
        # 3. Other commandAGI2 daemon instances
        ...
        # Add the image name
        run_cmd.append(f"commandagi2-daemon:{self.version}")
    ...
    def _setup_aws_ecs(self):
        ...
        response = self.ecs_client.run_task(
            cluster="commandagi2-cluster",
            taskDefinition="commandagi2-daemon",
            launchType="FARGATE",
            ...
            overrides={
                "containerOverrides": [
                    {
                        "name": "commandagi2-daemon",
                        "image": f"commandagi2-daemon:{self.version}",
                    }
                ]
            },
        )
        ...
        if self._task_arn:
            ...
            try:
                self.ecs_client.stop_task(
                    cluster="commandagi2-cluster", task=self._task_arn
                )
                ...
                while time.time() - start_time < self.timeout:
                    try:
                        response = self.ecs_client.describe_tasks(
                            cluster="commandagi2-cluster", tasks=[self._task_arn]
                        )
