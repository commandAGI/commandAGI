...
# Create the CLI app
cli = typer.Typer(help="commandAGI2 CLI")
...
@cli.command()
def version():
    """Display version information for commandAGI2."""
    typer.echo(f"commandAGI2 Version: {__version__}")
...
def screenshot(
    ...
    try:
        ...
        if computer == ComputerType.LOCAL_PYNPUT:
            ...
            computer_instance = DaemonClientComputer(
...
def grid_overlay(
    ...
    output_file: str = typer.Option(
        None, help="Output file path (defaults to input_grid.png)"
    ),
    ...
    if not processors_available:
        typer.echo(
            "Error: Processors module not available. Install with: pip install commandagi2[processors]"
        )
        return
...
def ocr(
    input_file: str = typer.Argument(..., help="Input screenshot file"),
    output_file: str = typer.Option(
        None, help="Output text file path (defaults to input.txt)"
    ),
):
    """Extract text from a screenshot using OCR."""
    if not processors_available:
        typer.echo(
            "Error: Processors module not available. Install with: pip install commandagi2[processors,pytesseract]"
        )
        return
...
def daemon(
    ...
    try:
        from commandAGI2.daemon.cli import start

        typer.echo(f"Starting daemon on port {port} with backend {backend}...")
        start(port=port, backend=backend)
    except ImportError:
        typer.echo(
            "Error: Daemon module not available. Install with: pip install commandagi2[daemon]"
        )
    except Exception as e:
        typer.echo(f"Error: {e}")
...
def run_example(
    example: str = typer.Argument(
        ..., help="Example file to run (e.g., 1_getting_started.py)"
    ),
):
...
def run_gym(
    ...
    if not gym_available:
        typer.echo(
            "Error: Gym module not available. Install with: pip install commandagi2[gym]"
        )
        return
