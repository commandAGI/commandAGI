...
class ParallelDriver(BaseDriver):
    ...
    def run_episode(
        ...
        try:
            ...
            while True:
                console.print(f"👉 [yellow]Step {step + 1}/{max_steps if max_steps is not None else '?'}[/]")
                ...
                for env_id in self.env.active_envs:
                    ...
                    console.print(f"💰 [green]Env {env_id} Reward: {rewards[env_id]}[/]")
        ...
        if return_episode:
            ...
            total_rewards = [episode.total_reward for episode in self.current_episodes.values()]
