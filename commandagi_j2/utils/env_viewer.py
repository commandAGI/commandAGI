...
class EnvironmentViewer:
    def __init__(self, env, refresh_rate=100, show_mouse=True, show_keyboard=True):
        ...
        self.keyboard_state_label = ttk.Label(self.info_frame, text="Keyboard State: N/A")
    ...
    def open_settings(self):
        ...
        ttk.Label(self.settings_window, text="Refresh Rate (ms):").grid(row=0, column=0, padx=5, pady=5, sticky="w")
        self.refresh_rate_var = tk.IntVar(value=self.refresh_rate)
        self.refresh_rate_scale = ttk.Scale(self.settings_window, from_=50, to=1000, orient="horizontal", variable=self.refresh_rate_var)
        ...
        self.mouse_check = ttk.Checkbutton(self.settings_window, text="Show Mouse State", variable=self.mouse_var)
        self.mouse_check.grid(row=1, column=0, columnspan=2, padx=5, pady=5, sticky="w")
        
        self.keyboard_check = ttk.Checkbutton(self.settings_window, text="Show Keyboard State", variable=self.keyboard_var)
        self.keyboard_check.grid(row=2, column=0, columnspan=2, padx=5, pady=5, sticky="w")
        
        # Apply button
        apply_button = ttk.Button(self.settings_window, text="Apply", command=self.apply_settings)
...
if __name__ == "__main__":
    from commandagi_j2.envs.computer_types import ScreenshotObservation, MouseStateObservation, KeyboardStateObservation, ComputerObservation, MouseButton, KeyboardKey
    import random
    ...
    class DummyEnv:
        def _get_observation(self):
            ...
            mouse_state = MouseStateObservation(buttons={MouseButton.LEFT: random.choice([True, False])}, position=(random.randint(0,800), random.randint(0,600)))
            keyboard_state = KeyboardStateObservation(keys={KeyboardKey.A: random.choice([True, False]), KeyboardKey.B: random.choice([True, False])})
            return ComputerObservation(screenshot=screenshot, mouse_state=mouse_state, keyboard_state=keyboard_state)
    
    env = DummyEnv()
    viewer = EnvironmentViewer(env, refresh_rate=100, show_mouse=True, show_keyboard=True)
    viewer.run() 