"""
Benchmarks for CommandLAB computer implementations.
...
def benchmark_computer(
    ...
    operations = {
        ...
        "mouse_move": lambda: computer.execute_mouse_move(MouseMoveAction(x=100, y=100, move_duration=0.1)),
        "mouse_scroll": lambda: computer.execute_mouse_scroll(MouseScrollAction(amount=10)),
        "click": lambda: computer.execute_click(ClickAction(x=100, y=100, move_duration=0.1, press_duration=0.1)),
        "type_short": lambda: computer.execute_type(TypeAction(text="test")),
        "type_long": lambda: computer.execute_type(TypeAction(text="This is a longer text to test typing performance")),
        "hotkey": lambda: computer.execute_keyboard_hotkey(KeyboardHotkeyAction(keys=[KeyboardKey.CTRL, KeyboardKey.A]))
    }
...
def plot_results(
    ...
    for i, operation in enumerate(operations):
        ...
        means = [statistics.mean(results[c][operation]) if operation in results[c] and results[c][operation] else 0 for c in computers]
        stds = [statistics.stdev(results[c][operation]) if operation in results[c] and len(results[c][operation]) > 1 else 0 for c in computers]
...
def main():
    parser = argparse.ArgumentParser(description='Benchmark CommandLAB computer implementations')
    parser.add_argument('--runs', type=int, default=5, help='Number of benchmark runs per operation')
    parser.add_argument('--plot', action='store_true', help='Generate plot of results')
    parser.add_argument('--operations', nargs='+', help='Specific operations to benchmark')
