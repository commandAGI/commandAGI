...
def plot_results(results: Dict[str, Dict[str, List[float]]], title: str = "Provisioner Benchmark") -> None:
    """
    ...
    setup_means = [statistics.mean(results[p]["setup"]) if results[p]["setup"] else 0 for p in provisioners]
    teardown_means = [statistics.mean(results[p]["teardown"]) if results[p]["teardown"] else 0 for p in provisioners]
    
    setup_stds = [statistics.stdev(results[p]["setup"]) if len(results[p]["setup"]) > 1 else 0 for p in provisioners]
    teardown_stds = [statistics.stdev(results[p]["teardown"]) if len(results[p]["teardown"]) > 1 else 0 for p in provisioners]
    ...
    rects2 = ax.bar(x + width/2, teardown_means, width, label='Teardown', yerr=teardown_stds)
...
def main():
    ...
    parser.add_argument('--no-teardown', action='store_true', help='Skip teardown benchmarking')
    parser.add_argument('--plot', action='store_true', help='Generate plot of results')
    args = parser.parse_args()
    
    # Define provisioners to benchmark
    provisioners = {
        "Docker (Local)": (DockerProvisioner, {
            "port": 8000,
    ...
    for name, (cls, kwargs) in provisioners.items():
        ...
        result = benchmark_provisioner(
            ...
            kwargs, 
            num_runs=args.runs, 
            include_teardown=not args.no_teardown
        )
        results[name] = result
