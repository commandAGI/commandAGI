...
def benchmark_daemon(
    port: int = 8000, computer_cls=LocalPynputComputer, num_runs: int = 10
) -> Dict[str, List[float]]:
    """
    ...
    endpoints = {
        "screenshot": {"endpoint": "/screenshot", "method": "GET", "data": None},
        "mouse_state": {"endpoint": "/mouse/state", "method": "GET", "data": None},
        "keyboard_state": {
        ...
        "hotkey": {
            ...
            "method": "POST",
            "data": KeyboardHotkeyAction(
                keys=[KeyboardKey.CTRL, KeyboardKey.A]
            ).model_dump(),
        },
    }
...
def plot_results(
    ...
    for i, endpoint in enumerate(endpoints):
        ...
        means = [
            (
                statistics.mean(results[c][endpoint])
                if endpoint in results[c] and results[c][endpoint]
                else 0
            )
            for c in computers
        ]
        stds = [
            (
                statistics.stdev(results[c][endpoint])
                if endpoint in results[c] and len(results[c][endpoint]) > 1
                else 0
            )
            for c in computers
        ]
...
def main():
    ...
    parser.add_argument(
        "--endpoints", nargs="+", help="Specific endpoints to benchmark"
    )
