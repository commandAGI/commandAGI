...
def generate_test_images(sizes: List[Tuple[int, int]]) -> Dict[str, Image.Image]:
    ...
    for width, height in sizes:
        ...
        # Draw some shapes
        draw.rectangle([width//10, height//10, width//3, height//3], outline='red', width=2)
        draw.ellipse([width//2, height//4, width*3//4, height*3//4], outline='blue', width=2)
        draw.line([width*4//5, height//10, width*9//10, height*9//10], fill='green', width=3)
...
def plot_results(
    ...
    title: str = "Image Utilities Benchmark"
) -> None:
    """
    ...
    for i, function in enumerate(functions):
        ...
        stds = [statistics.stdev(results[function][size]) if len(results[function][size]) > 1 else 0 for size in sizes]
...
def main():
    ...
    parser.add_argument('--runs', type=int, default=10, help='Number of benchmark runs per function')
