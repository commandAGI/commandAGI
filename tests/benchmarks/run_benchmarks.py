...
def main():
    parser = argparse.ArgumentParser(description="Run commandAGI benchmarks")
    ...
    parser.add_argument(
        "--screenparse-api-key", type=str, help="API key for ScreenParse.ai"
    )
    ...
    if "benchmark_screen_parser" in benchmarks and args.screenparse_api_key:
        benchmarks["benchmark_screen_parser"].extend(
            ["--api-key", args.screenparse_api_key]
        )
    ...
    with open(report_path, "w") as f:
        f.write(f"commandAGI Benchmark Summary\n")
        f.write(f"===========================\n\n")
