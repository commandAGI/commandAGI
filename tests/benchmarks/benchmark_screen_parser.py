...
try:
    from commandLAB.processors.screen_parser.screenparse_ai_screen_parser import (
...
def generate_test_images(
    sizes: List[Tuple[int, int]], text_densities: List[str]
    ...
    for width, height in sizes:
        ...
        for density in text_densities:
            ...
            draw.rectangle(
                [width // 10, height // 10, width // 3, height // 3],
                outline="gray",
                width=1,
            )
            draw.rectangle(
                [width * 2 // 3, height // 10, width * 9 // 10, height // 3],
                outline="gray",
                width=1,
            )
            ...
            draw.text(
                (width // 2 - 50, 10),
                f"Test Image {size_name} - {density}",
                fill="black",
                font=font,
            )
...
def benchmark_function(
    func: Callable, args: List[Any], num_runs: int = 5
) -> List[float]:
    """
        ...
        for density, image in density_images.items():
            ...
            if SCREENPARSE_AI_AVAILABLE and api_key:
                ...
                times = benchmark_function(
                    screenparse_ai_parse, [b64_image, api_key], num_runs
                )
...
def plot_results(
    ...
    fig, axes = plt.subplots(
        len(sizes), len(densities), figsize=(5 * len(densities), 4 * len(sizes))
    )
    ...
    for i, size in enumerate(sizes):
        for j, density in enumerate(densities):
            ax = (
                axes[i, j]
                if len(sizes) > 1 and len(densities) > 1
                else (
                    axes[j]
                    if len(sizes) == 1
                    else axes[i] if len(densities) == 1 else axes
                )
            )
            ...
            means = [
                (
                    statistics.mean(results[parser][size][density])
                    if size in results[parser] and density in results[parser][size]
                    else 0
                )
            ...
            stds = [
                (
                    statistics.stdev(results[parser][size][density])
                    if size in results[parser]
                    and density in results[parser][size]
                    and len(results[parser][size][density]) > 1
                    else 0
                )
                for parser in parsers
            ]
...
def main():
    ...
    if not PYTESSERACT_AVAILABLE and not (SCREENPARSE_AI_AVAILABLE and args.api_key):
        print(
            "Error: No screen parsers available. Please install pytesseract or provide a ScreenParse.ai API key."
        )
        return
